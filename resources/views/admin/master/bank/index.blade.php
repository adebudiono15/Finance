...
@section('content')
                                        ...
                                        <tr>
                                            ...
                                            <td class="text-center" style="width: 200px">
                                                ...
                                                <a href="#" data-id="{{ $item->id }}"
                                                    ...
                                                    <form action="{{ route('delete-bank', $item->id) }}"
        ...
        <div class="modal-dialog ">
            <div class="modal-content">
                <div class="modal-header bg-dark">
                    <h5 class="modal-title text-white">TAMBAH DATA BANK</h5>
                    <button type="button" class="close text-white" data-dismiss="modal">×</button>
                </div>
                <form class="form form-vertical" method="post" enctype="multipart/form-data"
                    action="{{ route('save-bank') }}">
                    @csrf
                    <div class="modal-body">
                        <div class="row">
                            <div class="col-lg">
                                <div class="form-group">
                                    <label for="nama_bank" @error('nama_bank') class="text-danger" @enderror>NAMA BANK @error('nama_bank')
                                        | {{ $message }}
                                        @enderror</label>
                                    <input type="text" class="form-control" value="{{ old('nama_bank') }}"
                                        name="nama_bank" placeholder="-" style="height: 28px;">
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    <div id="edit" class="modal fade" style="display: none;" aria-hidden="true">
        <div class="modal-dialog ">
            <div class="modal-content">
                <div class="modal-header bg-dark">
                    <h5 class="modal-title text-white">EDIT DATA BANK</h5>
                    <button type="button" class="close text-white" data-dismiss="modal">×</button>
                </div>
                ...
                error: function(err) {
                    ...
                    if (err.status == 422){
                        if (typeof(err_log.satuan) !== 'undefined'){
                            $('#edit').find('[name="bank"]').prev().html('<span style="color:red">Bank | '+err_log.divisi[0]+'</span>')
                        }else{
                            $('#edit').find('[name="nama_bank"]').prev().html('<span>Bank</span>')
                        }
                    }
                }
            })

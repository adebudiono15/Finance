...
@section('content')
                        ...
                        <div class="card-body">
                            <div class="table-responsive">
                                <table id="basic-datatables" class="display table table-striped table-hover" >
                                    ...
                                    <tbody>
                                        @foreach ($jenis_pengeluaran as $e=>$item)
                                        <tr>
                                            <td>{{ $e+1 }}</td>
        ...
        <div class="modal-dialog ">
            <div class="modal-content">
                <div class="modal-header bg-dark">
                    <h5 class="modal-title text-white">TAMBAH DATA PENGELUARAN</h5>
                    <button type="button" class="close text-white" data-dismiss="modal">×</button>
                </div>
                <form class="form form-vertical" method="post" enctype="multipart/form-data"
                    action="{{ route('save-pengeluaran') }}">
                    @csrf
                    <div class="modal-body">
                        <div class="row">
                            <div class="col-lg">
                                <div class="form-group">
                                    <label for="nama_jenis_pengeluaran" @error('nama_jenis_pengeluaran') class="text-danger" @enderror>NAMA JENIS PENGELUARAN @error('nama_jenis_pengeluaran')
                                        | {{ $message }}
                                        @enderror</label>
                                    <input type="text" class="form-control" value="{{ old('nama_jenis_pengeluaran') }}"
                                        name="nama_jenis_pengeluaran" placeholder="-" style="height: 28px;">
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    <div id="edit" class="modal fade" style="display: none;" aria-hidden="true">
        <div class="modal-dialog ">
            <div class="modal-content">
                <div class="modal-header bg-dark">
                    <h5 class="modal-title text-white">EDIT DATA PENGELUARAN</h5>
                    <button type="button" class="close text-white" data-dismiss="modal">×</button>
                </div>
        ...
        $('.btn-update').on('click', function() {
            ...
            $.ajax({
                ...
                success: function(data) {
                    // console.log(data)
                    $('#edit').modal('hide')
                    window.location.assign('/jenis-pengeluaran')
                },
                error: function(err) {

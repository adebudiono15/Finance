...
@section('title', 'Jenis Pendapatan')
...
@section('content')
                        ...
                        <div class="card-body">
                            <div class="table-responsive">
                                <table id="basic-datatables" class="display table table-striped table-hover" >
                                    ...
                                    <tbody>
                                        @foreach ($jenis_pendapatan as $e=>$item)
                                        <tr>
                                            <td>{{ $e+1 }}</td>
                                            <td>{{ $item->nama_jenis_pendapatan }}</td>
                                            <td class="text-center" style="width: 200px">
                                                ...
                                                <a href="#" data-id="{{ $item->id }}"
                                                    ...
                                                    <form action="{{ route('delete-pendapatan', $item->id) }}"
        ...
        <div class="modal-dialog ">
            <div class="modal-content">
                <div class="modal-header bg-dark">
                    <h5 class="modal-title text-white">TAMBAH DATA PENDAPATAN</h5>
                    <button type="button" class="close text-white" data-dismiss="modal">×</button>
                </div>
                <form class="form form-vertical" method="post" enctype="multipart/form-data"
                    action="{{ route('save-pendapatan') }}">
                    @csrf
                    <div class="modal-body">
                        <div class="row">
                            <div class="col-lg">
                                <div class="form-group">
                                    <label for="nama_jenis_pendapatan" @error('nama_jenis_pendapatan') class="text-danger" @enderror>NAMA JENIS PENDAPATAN @error('nama_jenis_pendapatan')
                                        | {{ $message }}
                                        @enderror</label>
                                    <input type="text" class="form-control" value="{{ old('nama_jenis_pendapatan') }}"
                                        name="nama_jenis_pendapatan" placeholder="-" style="height: 28px;">
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    <div id="edit" class="modal fade" style="display: none;" aria-hidden="true">
        <div class="modal-dialog ">
            <div class="modal-content">
                <div class="modal-header bg-dark">
                    <h5 class="modal-title text-white">EDIT DATA PENDAPATAN</h5>
                    <button type="button" class="close text-white" data-dismiss="modal">×</button>
                </div>
...
<script>
      $('.btn-edit').on('click', function() {
        ...
        $('.btn-update').on('click', function() {
            ...
            $.ajax({
                url: `/pendapatan/update/${id}`,
                method: "PATCH",
                data:formData,
                success: function(data) {
                    // console.log(data)
                    $('#edit').modal('hide')
                    window.location.assign('/jenis-pendapatan')
                },
                error: function(err) {
                    ...
                    if (err.status == 422){
                        if (typeof(err_log.satuan) !== 'undefined'){
                            $('#edit').find('[name="nama_jenis_pendapatan"]').prev().html('<span style="color:red">Jenis pendapatan | '+err_log.divisi[0]+'</span>')
                        }else{
                            $('#edit').find('[name="nama_jenis_pendapatan"]').prev().html('<span>Jenis pendapatan</span>')
                        }
                    }
                }
            })
